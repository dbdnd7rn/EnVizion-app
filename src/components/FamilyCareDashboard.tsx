  ...
  return (
    <View style={{ gap: 14 }}>
      ...
      <Card
        onPress={
          snapshot?.attention[0]
            ? () => openAttention(snapshot.attention[0])
            : onOpenTasks
        }
        label={
          attentionCount
            ? "Open the highest priority care item"
            : "Open today’s care work"
        }
        style={{
          backgroundColor: attentionCount ? "#FFF8F7" : "#EAF4EF",
          borderColor: attentionCount ? "#E8BDC3" : "#D4E8DE",
          padding: 16,
        }}
      >
        <View style={S.between}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 11, flex: 1 }}>
            <View
              style={{
                ...
                height: 40,
                borderRadius: 13,
                backgroundColor: attentionCount ? C.redBg : "#DDEFE6",
                alignItems: "center",
                justifyContent: "center",
              }}
            >
              <Icon
                name={attentionCount ? "warning-outline" : "checkmark-circle-outline"}
                color={attentionCount ? C.rose : C.green}
                size={21}
              />
            </View>
            <View style={{ flex: 1, gap: 2 }}>
              ...
              <Txt style={S.small}>
                {snapshot?.attention[0]?.title ??
                  "No overdue or time-sensitive care-plan items are recorded."}
              </Txt>
            </View>
          </View>
          <Icon name="chevron-forward" color={attentionCount ? C.rose : C.green} size={18} />
        </View>
      </Card>
      ...
      <Pressable
        ...
        accessibilityLabel={showDetails ? "Hide full family care dashboard" : "Show full family care dashboard"}
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 9 }}>
          <Icon name={showDetails ? "contract-outline" : "options-outline"} size={19} />
          <Text style={[S.h3, { fontSize: 13 }]}>
            {showDetails ? "Hide dashboard details" : "Show full dashboard"}
          </Text>
        </View>
        <Icon name={showDetails ? "chevron-up" : "chevron-down"} size={17} color={C.muted} />
      </Pressable>
